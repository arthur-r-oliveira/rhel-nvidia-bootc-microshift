...
Examples (from repository root; or use the root ./build-bootc-disk.sh wrapper):
  ./build/build-bootc-disk.sh -t qcow2 localhost/microshift-nvidia-bootc-test:latest
  ./build/build-bootc-disk.sh -t qcow2 -t raw -o ./out localhost/microshift-nvidia-bootc-test:latest
  ./build/build-bootc-disk.sh -t bootc-installer -c ./bib-config.toml \\
      localhost/microshift-nvidia-bootc-test:latest
  ./build/build-bootc-disk.sh -t ami -- localhost/foo:latest \\
...
types=()
output_dir="${BIB_OUTPUT_DIR:-${REPO_ROOT}/bib-output}"
