...
Environment (optional defaults):
  ...
  Sign in first (interactive az login or ./azure-login-sp.sh with cloud/azure/.env.azure).
  To load defaults from .env.azure in your shell: set -a; source cloud/azure/.env.azure; set +a
  (keep .env.azure chmod 600 and never commit it).

Typical workflow (from repository root; root ./build-bootc-disk.sh / ./push-vhd-to-azure.sh wrappers work too):
  1. Build VHD:  ./build/build-bootc-disk.sh -t vhd -o ./bib-output YOUR_BOOTC_IMAGE
  2. Upload:     ./cloud/azure/push-vhd-to-azure.sh --vhd ./bib-output/vpc/disk.vhd \\
