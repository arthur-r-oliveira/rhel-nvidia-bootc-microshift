...
ARGFILE="${SCRIPT_DIR}/argfile.conf"
...
if [[ "${CONFIGURE_BUILD_HOST_EUS:-0}" == "1" ]] && [[ -n "${EUS_RELEASE:-}" ]]; then
	if ! command -v subscription-manager >/dev/null 2>&1; then
		...
		_CFG="${SCRIPT_DIR}/scripts/configure-host-rhsm-eus.sh"
		if [[ ! -x "${_CFG}" ]]; then
			chmod 755 "${_CFG}"
		fi
...
if [[ "${BUILD_WITH_HOST_RHSM:-}" == "1" ]]; then
	...
	if [[ "${_MOUNT_YUM}" -eq 1 ]]; then
		if [[ "${CONFIGURE_HOST_NVIDIA_REPOS:-1}" == "1" ]]; then
			_NVR="${SCRIPT_DIR}/scripts/configure-host-nvidia-build-repos.sh"
			if [[ ! -x "${_NVR}" ]]; then
				chmod 755 "${_NVR}"
			fi
		...
		if [[ "${CONFIGURE_HOST_MICROSHIFT_REPOS:-1}" == "1" ]]; then
			_MS="${SCRIPT_DIR}/scripts/configure-host-microshift-build-repos.sh"
			if [[ ! -x "${_MS}" ]]; then
				chmod 755 "${_MS}"
			fi
...
if [[ -f "${SCRIPT_DIR}/.pull-secret.json" ]]; then
	AUTHFILE="${SCRIPT_DIR}/.pull-secret.json"
	...
	echo "  - Place console.redhat.com pull secret at ${SCRIPT_DIR}/.pull-secret.json, or"
